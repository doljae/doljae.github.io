...
leetcode.com](https://leetcode.com/problems/determine-if-two-strings-are-close/)
...
leetcode.com](https://leetcode.com/problems/minimum-number-of-steps-to-make-two-strings-anagram/)
...
1.  문자열을 구성하는 문자 2개의 위치를 바꾼다
2.  문자열을 구성하는 특정한 문자 A, B에 대해서 A 문자를 모두 B로, B 문자를 모두 A로 변경한다.
    1.  예를 들어 "AABBCC"는 2번 규칙에 의해 "AACCBB"가 될 수 있다.
    2.  예를 들어 "AAB" 2번 규칙에 의해 "BAA"가 될 수 있다.
    3.  예를 들어 "AAB"는 2번 규칙에 의해 "AAC"가 될 수 없다. B를 C로 치환한 것이지만 C는 원래 문자열에 존재하지 않는 문자이기 때문이다.
...
![](https://blog.kakaocdn.net/dna/lJn3y/btrdHfkyB2e/AAAAAAAAAAAAAAAAAAAAAMKm3Y9oIRuPB_Ujxy3_OzbepI1q_gTj5zjG6MBP1Ncr/img.png?credential=yqXZFxpELC7KVnFOS48ylbz2pIh7yKj8&expires=1772290799&allow_ip=&allow_referer=&signature=RmEYIyRaFHsQMGstYGvnMBEX0Vc%3D)
